...
//
// RUN: Run --timeout_per_input=5 --batch_size=100 && SolutionIs FU
...
__attribute__((noinline)) void Puzzle(const char* data, size_t size) {
  // Guessing more than 2 bytes takes too long.
  // If this function is inlined, even 2 bytes take too long.
  if (size == 2 && data[0] == 'F' && data[1] == 'U') {
    abort();
  }
}
